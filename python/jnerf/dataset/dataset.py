...
@DATASETS.register_module()
class MyNerfDataset():
	def __init__(self, root_dir, para_path, batch_size, mode='train', aabb_scale=None, to_jt=True, have_img=True, preload_shuffle=True):
		self.root_dir = root_dir
		...
		self.image_data = self.image_data.detach()
	...
	def load_data(self,root_dir=None):
		...
		for frame in tqdm(image_path):
			if self.have_img:
				...
				img = read_image(img_path)
				self.H.append(img.shape[0])
				self.W.append(img.shape[1])
				self.image_data.append(img.reshape(-1, 3))
			else:
		...
		if self.image_data.shape[-1]==3:
			self.image_data=jt.concat([self.image_data, jt.ones(self.image_data.shape[:-1]+(1,))],-1).stop_grad()

		self.shuffle_index=jt.randperm(self.image_data.shape[0]).detach()
